...
export function PokemonDetail({ pokemon, onClose }: Props) {
  ...
  const weaknesses = Object.entries(profile)
    .filter(([, v]) => v > 1)
    .sort((a, b) => b[1] - a[1]);
  const resistances = Object.entries(profile)
    .filter(([, v]) => v < 1)
    .sort((a, b) => a[1] - b[1]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" onClick={onClose}>
      <div
        className="relative bg-gray-900 rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-gray-700"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div
          className="relative h-48 rounded-t-3xl flex items-end px-6 pb-4"
          style={{ background: `linear-gradient(135deg, ${primaryColor}55, ${primaryColor}22)` }}
        >
          <button
            onClick={onClose}
          ...
          <img
            src={artwork ?? ''}
            alt={pokemon.name}
            className="absolute right-6 bottom-0 w-40 h-40 object-contain drop-shadow-2xl"
          />
          <div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {pokemon.abilities.map(a => (
                ...
                >
                  {a.name.replace(/-/g, ' ')}
                  {a.isHidden && <span className="ml-1 text-xs text-purple-400">(HA)</span>}
                </span>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {pokemon.stats.map(s => (
                <StatBar key={s.name} name={s.name} value={s.base} />
              ))}
            </div>
          </div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {weaknesses.map(([type, mult]) => (
                    <span key={type} className="relative">
                      ...
                      <span className="ml-0.5 text-xs text-red-400 font-bold">×{mult}</span>
                    </span>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {resistances.map(([type, mult]) => (
                    <span key={type} className="relative">
                      <TypeBadge type={type} size="sm" />
                      <span className="ml-0.5 text-xs text-green-400 font-bold">×{mult}</span>
                    </span>
                  ))}
                </div>
              </div>

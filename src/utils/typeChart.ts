// Effectiveness multipliers: attacking type → defending type → multiplier
...
export function getDefensiveProfile(types: string[]): Record<string, number> {
  ...
  for (const atk of ALL_TYPES) {
    ...
    if (eff !== 1) profile[atk] = eff;
  }
  return profile;
}

export function getTeamWeaknessMatrix(
  memberTypes: string[][]
  ...
  for (const types of memberTypes) {
    for (const atk of ALL_TYPES) {
      const eff = getEffectiveness(atk, types);
      if (eff > 1) counts[atk] = (counts[atk] ?? 0) + 1;
    }
  }
  return counts;
}
